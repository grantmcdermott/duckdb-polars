...
The new(ish) kid on the block is **duckplyr**
([announcement](https://duckdb.org/2024/04/02/duckplyr.html) /
[homepage](https://duckplyr.tidyverse.org/)).
...
All of this is exciting and I expect that **duckplyr** will become the default
R -> DuckDB interface for many users... and perhaps their default data wrangling
tool fullstop. There are still a few rough edges IMO (e.g., factor variables
aren't fully supported [yet](https://github.com/tidyverse/duckplyr/issues/88)).
But the basics are certainly all there. For example:

```{r duckplyr_q1}
library(duckplyr, warn.conflicts = FALSE)

read_parquet_duckdb("nyc-taxi/**/*.parquet") |>
  summarise(
    mean_tip = mean(tip_amount),
    .by = passenger_count
  )
